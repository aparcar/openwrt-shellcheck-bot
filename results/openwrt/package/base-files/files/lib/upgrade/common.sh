...
In openwrt/package/base-files/files/lib/upgrade/common.sh line 163:
			for disk in $(find /dev -type b); do
                                    ^------------------^ SC2044: For loops over find output are fragile. Use find -exec or a while read loop.


In openwrt/package/base-files/files/lib/upgrade/common.sh line 164:
...
In openwrt/package/base-files/files/lib/upgrade/common.sh line 181:
...
In openwrt/package/base-files/files/lib/upgrade/common.sh line 189:
		while read line; do
                      ^--^ SC2162: read without -r will mangle backslashes.
...
In openwrt/package/base-files/files/lib/upgrade/common.sh line 229:
	if [ -b "$disk" -o -f "$disk" ]; then
                        ^-- SC2166: Prefer [ p ] || [ q ] as [ p -o q ] is not well defined.


In openwrt/package/base-files/files/lib/upgrade/common.sh line 241:
		part_magic_efi "$disk" && {
                                       ^-- SC2015: Note that A && B || C is not if-then-else. C may run when A is true.


In openwrt/package/base-files/files/lib/upgrade/common.sh line 245:
